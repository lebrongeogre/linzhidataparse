 ...
 * 用于批量裁剪栅格、并输出文件
 */
public class ClipRasterProcessMODIS extends BaseJob {

    private static final Logger logger = Logger.getLogger(ClipRasterProcessMODIS.class);
    ...
    public void startJob(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        ...
        Map<String,String> uviClipMap = (Map<String, String>) config.get("MODIS_AOD_Clip");
        //获取配置文件中待裁剪栅格文件的路径
        ...
        if (fileList.size() != 0) {
            ...
            for (String tempFile : fileList) {
                ...
                try{
                    ...
                    int ref = baseMethod.clipRaster(slPath, tempFile, outTifPath,dateStr);
