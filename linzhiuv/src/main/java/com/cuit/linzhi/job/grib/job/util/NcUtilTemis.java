package com.cuit.linzhi.job.grib.job.util;

import com.cuit.job.utils.metegrib2.DataGrid;
import com.cuit.job.utils.metegrib2.GridPoint;

import com.cuit.linzhi.job.grib.job.vo.NcBasicMeta;
...
public class NcUtilTemis {
    ...
    public static NcBasicMeta getSPHDNcDataMetaData(String file, String paramName) {
        ...
        try {
            ...
            //读取经度、纬度、数据数组
            try {
                float[] lngList = readVarTo1Array(ncfile, ln);
                float[] latList = readVarTo1Array(ncfile, lat);

                float[][] dataList = readVarToNArray(ncfile, paramVar);

                List<GridPoint> gridPoints = getGridPointList(dataList,latList,lngList);
                ...
                if (lngList != null && lngList.length >= 0 && latList != null && latList.length > 0) {
                    ...
                    ncBasicMeta.setLngLists(lngList);
    ...
    private static float[][] readVarToNArray(NetcdfFile ncFile, Variable variable) throws IOException, InvalidRangeException {
        ...
        data = (short[][]) ((short[][]) variable.read(origin, size).reduce().copyToNDJavaArray());
